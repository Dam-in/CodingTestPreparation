# 1213 팰린드롬 만들기
import sys
name = sorted(sys.stdin.readline().strip(), reverse=True)
N = len(name)
...
if len(List) == 1:
    ...
    name.remove(List[0])
for i in range(N//2):
    palindrom[i] = name.pop()
    palindrom[-(1+i)] = name.pop()
...
# 2등
'''
p = 'ABCDEFGHIJKLMNOPQRSTUVWXYZ';l = [0] * 26;k = 0;t = ''
